<template>
  <div v-if="showAddEmail" id="add-email-container">
    <div id="add-email">
      <i class="el-icon-circle-close" id="close-add-email" @click="close"></i>
      <span class="text-link" id="to-user-space" @click="toUserSpace">>>个人信息</span>
      <div class="form-item">
        <label for="email" style="top: 12.52%">输入邮箱</label>
        <input type="text" id="email" name="email" v-model="email" style="top: 18.49%"/>
      </div>
      <div class="form-item">
...
<script>
import {checkCodeToEmail, sendCodeToEmail} from "@/network/register";
...
export default {
  ...
  methods: {
    toUserSpace() {
      this.$store.commit('addEmailToUserSpace')
      this.reset()
    },
    close() {
      this.$store.commit('closeAddEmail')
      this.reset()
    },
    reset() {
      this.email = ''
  ...
  computed: {
    showAddEmail() {
      return this.$store.getters.showAddEmail
    },
    codeMsg() {
...
#add-email-container i {
  position: absolute;
}

#add-email-container i:hover {
  cursor: pointer;
}
